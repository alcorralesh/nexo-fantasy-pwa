...
function AdminView({ marketRules, setMarketRules, clubRules, setClubRules, economyRules, setEconomyRules, settlementRules, setSettlementRules, onboardingConfig, onForceOnboarding, legalConfig, onPublishLegalVersion, scoringRules, onChangeScoringRules, teams, leagues, participations, squads, bids, playerContracts, playerOffers, sentOffers, playerCatalog, catalogSyncEnabled, onCatalogSyncApplied, onSavePlayer, fantasyEvents, onCreateFantasyEvent, onSnapshotFantasyEvent, onOpenLeague, onRenewMarket, notify }: {
  ...
  onCatalogSyncApplied: () => Promise<void>;
...
function CatalogSyncAdminPanel({ enabled, onApplied, notify }: { enabled: boolean; onApplied: () => Promise<void>; notify: (value: string) => void }) {
  ...
  async function refreshHistory() {
    if (!enabled) return;
    try { setHistory(await loadNexoPlayerCatalogSyncHistory()); } catch { /* La sincronización sigue disponible aunque falle el historial. */ }
  }
  ...
  async function run(mode: "preview" | "apply") {
    ...
    try {
      ...
      setPreview(result);
      if (mode === "apply") { await onApplied(); notify(`Catálogo actualizado: ${result.summary.total} jugadores`); }
      await refreshHistory();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "No se pudo completar la sincronización"); }
    finally { setBusy(null); }
  }

  const summary = preview?.summary;
  return <section className="admin-panel catalog-sync-panel"><header><div><p className="eyebrow">FUENTES OFICIALES · SINCRONIZACIÓN</p><h2>Actualizar plantillas</h2><p>Comprueba LALIGA y Liga F desde el backend. Las fichas que desaparecen se desactivan sin borrar su historial.</p></div><span className="catalog-source-lock">PROTEGIDO</span></header><div className="catalog-source-grid"><span><b>LALIGA</b><small>Primera y Segunda · plantillas públicas</small></span><span><b>Liga F</b><small>16 clubes · plantillas públicas</small></span><span><b>Valores</b><small>Los ajustes manuales se conservan</small></span></div>{!enabled && <article className="catalog-sync-demo-note">Inicia sesión con una cuenta administradora real para consultar las fuentes y actualizar Supabase.</article>}{summary && <div className="catalog-sync-summary"><span><small>NUEVOS</small><strong>+{summary.additions}</strong></span><span><small>CAMBIOS</small><strong>{summary.updates}</strong></span><span><small>BAJAS</small><strong>{summary.deactivations}</strong></span><span><small>TOTAL</small><strong>{summary.total}</strong></span><span><small>POR DIVISIÓN</small><strong>{summary.competitions.primera} · {summary.competitions.segunda} · {summary.competitions.liga_f}</strong></span></div>}{error && <p className="form-error">{error}</p>}<div className="catalog-sync-actions"><button className="secondary-button" disabled={!enabled || busy !== null} onClick={() => run("preview")}>{busy === "preview" ? "Consultando…" : "Comprobar cambios"}</button><button className="primary-button" disabled={!enabled || busy !== null || !preview} onClick={() => run("apply")}>{busy === "apply" ? "Actualizando…" : "Aplicar actualización"}</button></div>{history.length > 0 && <div className="catalog-sync-history"><p className="eyebrow">ÚLTIMAS EJECUCIONES</p>{history.slice(0, 4).map((job) => <article key={job.id}><span className={job.status}>{job.status === "succeeded" ? "✓" : job.status === "running" ? "↻" : "!"}</span><p><strong>{job.mode === "apply" ? "Actualización" : "Vista previa"}</strong><small>{new Date(job.started_at).toLocaleString("es-ES")} · {job.catalog_version ?? "sin versión"}</small></p><b>{job.status === "succeeded" ? `${job.summary.total ?? 0} jugadores` : job.status === "running" ? "En curso" : "Fallida"}</b></article>)}</div>}</section>;
}
