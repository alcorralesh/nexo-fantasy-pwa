...
export type CareerLabPlayer = { id: string; name: string; initials: string; position: string; club: string; clubId: string; value: number; photoUrl?: string; original: boolean; active: boolean; status?: string; points?: number };
...
export type CareerLabState = {
  ...
  state: { budget: number; confidence: number; reputation: number; sportingPoints: number; objectivePoints: number; consecutiveFailures: number; dismissalThreshold?: number; status: string; squad: CareerLabPlayer[]; market?: CareerLabPlayer[]; currentLineup?: { formation: string; captainId: string; originals?: number; valid: boolean; locked: boolean; delegated?: boolean; delegationPlan?: string; points?: number; players: CareerLabPlayer[] } | null; reports: Array<Record<string, unknown>>; decisions: Array<Record<string, unknown>>; objectives: CareerLabObjective[]; decisionPrompt?: CareerLabDecisionPrompt; incidents: Array<Record<string, unknown>>; interludes: Array<Record<string, unknown>>; delegation?: { used: number; maximum: number; remaining: number; cooldownMatchdays: number; nextAvailableMatchday: number; current?: { matchday: number; plan: string; cost: number } | null }; calendarExceptions: Array<Record<string, unknown>>; calendar?: { current?: CareerLabCalendarRound | null; next?: CareerLabCalendarRound | null }; activeInterlude?: Record<string, unknown>; realSideEffects: number };
...
export type CareerLabPublicPreview = Pick<CareerLabState, "state"> & {
  session: Pick<CareerLabSession, "title" | "userName" | "competitionId" | "sportsClubName" | "difficulty" | "status" | "matchday" | "maximumMatchday" | "phase" | "updatedAt">;
};
...
export type CareerLabPublicAction = "decision" | "prepare_lineup" | "save_lineup" | "lock_lineup" | "buy" | "sell" | "interlude" | "interlude_plan" | "interlude_project" | "interlude_story" | "incident" | "delegate";
export const actCareerLabPublic = (token: string, action: CareerLabPublicAction, payload: Record<string, unknown> = {}) => rpc<CareerLabPublicPreview>("manager_career_lab_public_action", { target_token: token, target_action: action, target_payload: payload });
