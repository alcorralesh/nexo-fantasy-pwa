...
Deno.serve(async (request) => {
  ...
  try {
    ...
    if (profile?.role !== "admin") return json({ error: "Acceso reservado a administradores" }, 403);
    ...
    if (jobError) return json({ error: jobError.code === "23505" ? "Ya hay una sincronización en curso" : jobError.message }, jobError.code === "23505" ? 409 : 500);
    jobId = job.id;
    ...
    if (mode === "apply") {
      ...
      return json({ jobId, mode, catalogVersion, summary: data });
    }
    ...
    return json({ jobId, mode, catalogVersion, summary });
  } catch (error) {
